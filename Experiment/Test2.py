...
async def process_help_command(message: Message):
    await message.answer(
        text='Я бот, демонстрирующий как работает разметка '
             ...
             '/iu \- наклонный подчеркнутый текст\n'
             '/biu \- жирный наклонный подчеркнутый текст\n'
        # '/test2\ - тестовый пример\n'
        # '/test5\ - тестовый пример'
    )


# Этот хэндлер будет срабатывать на команду "/test2"
@dp.message(Command(commands='test2'))
async def process_test2_command(message: Message):
...
@dp.message(Command(commands='test5'))
async def process_test5_command(message: Message):
    await bot.send_message(
        chat_id='1521587460',
